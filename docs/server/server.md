...
| [`__tests__/enum-sync.test.ts`](../../server/src/__tests__/enum-sync.test.ts) | Verifies enums in `server/src/types.ts` stay in sync with `lib/enums.ts` |
...
Requires `DATABASE_URL` in `.env` (local) or server environment:
...
The server runs on a VDS accessible at `https://incuby.duckdns.org` via Nginx reverse proxy with Let's Encrypt TLS. The process is managed by pm2 under the name `hives`.
...
Nginx terminates TLS on port 443 and proxies all traffic (HTTP and WebSocket) to `localhost:3000`. Certificates managed by Certbot / Let's Encrypt with automatic renewal.
