...
|------|---------|-------------|
| [lib/enums.ts](../../lib/enums.ts) | `BuildingType`, `PlayerRank`, `PlayerRole`, `Faction`, `UnitType` | Game-wide enum definitions; imported by client and admin via Vite's TypeScript handling |
...
| Value | Description |
|---|---|
| `INCUBATOR` | Zombie faction HQ |
| `HIVE` | Zombie faction base |
| `SHELTER_ZOMBIE` | Zombie shelter |
| `MUTATOR` | Zombie research building |
| `EXTRACTOR` | Zombie resource building |
| `MILITARY_BASE` | Human faction HQ |
| `RESISTANCE_BASE` | Human faction base |
| `SHELTER_HUMAN` | Human shelter |
| `LABORATORY` | Human research building |
| `TRAINING_BASE` | Human training building |

#### PlayerRank

5 progression levels: `NOVICE`, `SURVIVOR`, `VETERAN`, `ELITE`, `GENERAL`

#### PlayerRole

3 special roles: `QUEST_MASTER`, `NPC`, `BOSS`

#### Faction

3 factions: `HUMANS`, `ZOMBIES`, `NEUTRAL`

#### UnitType

4 unit types: `HUMAN_A`, `HUMAN_B`, `ZOMBIE_A`, `ZOMBIE_B`

#### Note on server duplication

The server duplicates these enums in `server/src/types.ts` due to a `rootDir: "./src"` tsconfig constraint that prevents importing from outside `src/`. A sync test at [`server/src/__tests__/enum-sync.test.ts`](../../server/src/__tests__/enum-sync.test.ts) verifies the two copies stay in sync.
